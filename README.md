...
### 👉 **Linux安装**
   ...
   ```sh
bash <(curl -s -L https://raw.githubusercontent.com/CakeSystem/CakeMinerSystem/main/install.sh)
   ```
...
(管理员端邮箱收益小额自助提币在CakeMinerSystem后台 - 矿池节点 - 自助提币处进行提币)
...
The small amount of self-withdrawal from the administrator's email can be made in the CakeMinerSystem backend - Mining Pool node - Self-withdrawal area.
...
[Важное обновление] Добавлен защитный патч, атакованный CC
   ...
   [!!!注意：连接池模式需要对应接入的CakeMinerSystem服务端版本 >= 3.8.0]
